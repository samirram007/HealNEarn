...
const Profile = () => {
  const userProfile = useUserprofile(); // Get user profile query instance
        ...
        <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5 ">
          <div className="relative z-30 mx-auto -mt-22 h-30 w-full max-w-30 rounded-full bg-white/20 p-1 backdrop-blur sm:h-44 sm:max-w-44 sm:p-3">
            <div className="relative drop-shadow-2  ">
              <img src={emptyUserImage} alt="profile" className='rounded-circle ' />
              <label
                htmlFor="profile"
          ...
          <div className="mt-4 flex flex-col items-center gap-2">
            ...
            <h3 className="mb-1.5 text-2xl font-semibold text-black dark:text-white">
              {userProfile.data?.data.data.name}
            </h3>
            <p className="font-medium uppercase"> Role: {userProfile.data?.data.data.role}</p>
            <p className="font-medium uppercase"> CODE: {userProfile.data?.data.data.username}</p>
            <div className=' font-bold    cursor-pointer btn-link top-12 left-1  '  >
              <span className='   text-teal-500 uppercase'>
                <div className="text-[1rem] flex flex-row   justify-center items-center gap-2">
                  <div>Status:</div>
                  <div>

                    {
                      userProfile.data?.data.data.status === 'active' ?
                        <>
                  ...
                  <div>
                    ...
                    {userProfile.data?.data.data.status}
                  </div>
            ...
            <div className='relative my-6 flex flex-row justify-center items-center gap-4 md:gap-12  
                                 text-center uppercase   '>

              <div className=' text-[0.7rem] flex flex-col justify-start items-center'>

                <div className='text-[0.7rem] text-teal-500'>SPONSER</div>
                <div>
                  <div className='w-12 h-12 rounded-full overflow-clip'>
                    <img className='w-full h-full object-cover' src={emptyUserImage} alt="" />
                  </div>
                </div>
                <div className='text-[0.7rem]'> {userProfile.data?.data.data.parent?.name}</div>
                <div className=' text-[0.8rem] border-2 border-teal-500   font-bold px-2 rounded-lg'>CODE: <span onClick={() => handleCopyClick(userProfile.data?.data.data.parent?.username)} className='text-[0.8rem]'> {userProfile.data?.data.data.parent?.username}</span></div>

              </div>
              <div className='text-[0.7rem] flex flex-col justify-start items-center'>
                <div className='text-[0.7rem] text-teal-500'>MANAGER</div>
                <div>
                  <div className='w-12 h-12 rounded-full overflow-clip'>
                    <img className='w-full h-full object-cover' src={emptyUserImage} alt="" />
                  </div>
                </div>
                <div className='text-[0.7rem]'> {userProfile.data?.data.data.manager?.name}</div>
                <div className=' text-[0.8rem] border-2 border-teal-500   font-bold px-2 rounded-lg'>CODE: <span onClick={() => handleCopyClick(userProfile.data?.data.data.manager?.username)} className='text-[0.8rem]'> {userProfile.data?.data.data.manager?.username}</span></div>
