...
const ReferralBlock = ({ initialValues, setInitialValues, isChanging, setChanging }) => {
    ...
    useEffect(() => {
        ...
        if (userProfile.data?.data.data.userType === 'manager') {
            setInitialValues(prev => (
                {
                    ...prev,
                    managerId: userProfile.data?.data.data.id,
                    parentId: userProfile.data?.data.data.id,
                }
            ))
            setParent(userProfile.data?.data.data)
            setManager(userProfile.data?.data.data)
        }
        else if (userProfile.data?.data.data.userType === 'member') {
            setInitialValues(prev => (
                {
                    ...prev,
                    managerId: userProfile.data?.data.data.managerId,
                    parentId: userProfile.data?.data.data.id,
                }
            ))
            setParent(userProfile.data?.data.data)
            setManager(userProfile.data?.data.data.manager)
        }
                ...
                setInitialValues(prev => (
                    {
                        ...prev,
                        parentId: _parent.id,
                        managerId: _parent.managerId,
                    }
                ))
                setParent(prev => _parent)
                setManager(prev => _parent.manager)
                setChanging(false)
            }
            else {
                const payload = {
